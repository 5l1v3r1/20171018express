	  ...
	  if (response.data.code ==200){
		...
		for(var i=0;i<response.data.user.length;i++){
			...
			for(var j=0;j<items.length;j++){
				...
				var d = new Date(Date.parse(items[j].enddate.replace(/-/g,   "/").replace(/T/g,   " ").replace(/Z/g,   "")));
				var littledate = new Date(d.getFullYear, d.getMonth() - self.state.datestep, d.getDate());
				console.log(d);
				console.log(nowdate);
  ...
  renderSearchTable(data) {
    ...
    return(
      <MuiThemeProvider>
      	  ...
      	  <div>
	      Date:
		  <DropDownMenu value={this.state.category} onChange={(event,index,value)=>this.handleDateStepChange(value)}>
